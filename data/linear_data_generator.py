...
def generate_synthetic_linear_model_with_uniform_distr_samples(noise: float, m: int, N: int, seed: int = 17171,
                                                               a: float = None, b: float = None) -> List[List[Data]]:
    # data generation
    data = []
    for i in range(N):
        data_i, _, _, _ = _generate_synthetic_linear_model_sample_with_uniform_distr(a, b, noise, m)
        data += [data_i]
    return data
...
def generate_synthetic_linear_model_with_cap_normal_distr_samples(noise: float, m: int, N: int, a: float, b: float,
    ...
    data = []
    for i in range(N):
        data_i, _, _, _ = _generate_synthetic_linear_model_sample_with_cap_normal_distr(a, b, noise, m, max_val, std)
        data += [data_i]
    return data
