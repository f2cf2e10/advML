...
def test_gd_lm_l2_loss():
    ...
    tol = 1E-8
    model0 = LinearRegression(np.array([1.0, 0.0]), False)
    model_opt = LinearRegression(np.array([a, b]), False)
    loss = L2()
...
def test_gd_lm_l2_loss_l1_const():
    ...
    data = [{'x': x[i, :], 'y': y[i]} for i in range(n)]
    tol = 1E-8
    model0 = LinearRegression(np.array([0.5, 0.5]), False)
    model_opt = LinearRegression(np.array([a, b]), False)
    loss = L2()
    proj = norm.L1
    const = 1.0
...
def test_gd_log_ce_loss():
    ...
    tol = 1E-6
    model0 = LogisticRegression(np.array([1.0, 0.0]), False)
    model_opt = LogisticRegression(np.array([a, b]), False)
    loss = CrossEntropy()
